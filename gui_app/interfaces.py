class AI_API:
    ...
    def __init__(self, main_model="deepseek-v3", weather_model="gpt-4o-mini"):
...
# deepseek-v3.2
    ...
    def chat_with_model(self, message: str) -> str:
        system_prompt = (
            ...
            "切记，每次回复维修相关的解决办法时，必须在文本的最后一段提示用户以下内容：\"联系维修人员或者去线下门店找专业维修人员进行维修\"，"
            ...
            "配合霍尔编码器+PID闭环控制，割苗高度调节精度±10mm，单株作业耗时≤8s。太阳能蓄电池驱动，适配行距30cm以上地块，"
            "割苗准确率≥85%，锌肥利用率提升80%以上，作业效率达人工的2倍，设备成本控制在1500元左右，"
            "为玉米苗期精准化、轻简化管理提供低成本解决方案。"
            "你的服务对于用户是免费的,不需要用户为此支付费用"
            "当用户询问你割苗情况时,请尝试分析"
        )
